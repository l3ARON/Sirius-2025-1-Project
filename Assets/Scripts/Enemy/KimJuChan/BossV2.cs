...
public class BossV2 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //공격 패턴 저장
        attackPatterns = new IEnumerator[][]{
            new IEnumerator[] { Taunt() },
            new IEnumerator[] { BreathShot() },
            new IEnumerator[] { Rush(), Magic() }
        };
    }
    ...
    void FixedUpdate()
    {
        ...
        if (isChase)
        {
            ...
            // 이동 방향에 따라 회전 방향 설정
            if (dirX < 0)
                transform.rotation = Quaternion.Euler(0, 180, 0); // 왼쪽 바라봄
            else
                transform.rotation = Quaternion.Euler(0, 0, 0);   // 오른쪽 바라봄
        }
        if (!isDetec)
        {
    ...
    IEnumerator Think()
    {
        ...
        {
            //randomAttack = Random.Range(0, attackPatterns[0].Length);
            randomAttack = 0;
            StartCoroutine(attackPatterns[0][randomAttack]);
        }
        //중거리
        else if (distance <= middleDetec)
        {
            //randomAttack = Random.Range(0, attackPatterns[1].Length);
            StartCoroutine(attackPatterns[1][randomAttack]);
        }
        //원거리
        else if (distance <= outlineDetec)
        {
            //randomAttack = Random.Range(0, attackPatterns[2].Length);
            randomAttack = 0;
            StartCoroutine(attackPatterns[2][randomAttack]);
        }
        else
        {
            isDetec = false;
